...
#define JSONRPC_ITEM_NAME "jsonrpc"
...
#define RETURN_ON_NULL(v, ret_val)                                             \
    {                                                                          \
        if (v == NULL)                                                         \
        {                                                                      \
            return ret_val;                                                    \
...
#define SET_ITEM(rpc_json, s, i)                                               \
    (cJSON_AddItemToObject(rpc_json->rpc_json_obj, s, i))
...
/**
 * @brief		Creates RPC-JSON request. Add parameters using
 * rpc_json_add_parameter_int/string functions.
 ...
 *
 * @return 		True if request is successfully created. False
 * otherwise.
 */
 ...
 * otherwise.
 */
bool rpc_json_create_response_from_json_object(RPCJson_t* obj, int id,
                                               cJSON* result)
{
    obj->type = RPC_JSON_RESPONSE;
    ...
    SET_ITEM(obj, JSONRPC_ITEM_NAME, json_rpc_item);
    SET_ITEM(obj, ID_ITEM_NAME, id_item);
    SET_ITEM(obj, RESULT_ITEM_NAME, result);
...
/**
 * @brief		Creates RPC-JSON notification object. Add parameters
 * using rpc_json_add_parameter_int/string functions.
 * @param[out]	obj		-	created object handle
 ...
 *
 * @return 		True if parameter is successfully added. False
 * otherwise.
 */
bool rpc_json_add_parameter_string(RPCJson_t* obj, const char* name,
                                   const char* value)
{
 ...
 *
 * @return 		If string representations is successfully created
 * returns pointer to a char string which MUST BE CLEARED from the outside.
 * 				Otherwise return NULL.
 */
char* rpc_json_print(RPCJson_t* obj) { return cJSON_Print(obj->rpc_json_obj); }

/**
 * @brief		Creates rpc-json object from a given string.
 * @param[out]	obj		-	object handle
 * @param[in]	str		-	string representation of a RPC-JSON
 * call.
 *
 ...
 */
RPCJson_type_t rpc_json_get_obj_type(RPCJson_t* obj) { return obj->type; }
...
{
    if (obj->type != RPC_JSON_REQUEST && obj->type != RPC_JSON_ERROR &&
        obj->type != RPC_JSON_RESPONSE)
    {
        return false;
    }
 ...
 * @param[in]	obj		-	object handle.
 *
 * @return 		pointer to a string in case of value found. NULL
 * otherwise.
 */
 ...
 * @param[in]	obj		-	object handle
 * @param[in]	name	-	name of the parameter
 * @param[out]	i		-	pointer to where parameter value will be
 * saved.
 *
 ...
 * @param[in]	name	-	name of the parameter
 *
 * @return 		pointer to a string if parameter is found. false
 * otherwise.
 */
 ...
 */
void rpc_json_delete(RPCJson_t* obj) { cJSON_Delete(obj->rpc_json_obj); }
 ...
 *
 * @return	pointer to 'params' object in case of successful allocation.
 * NULL otherwise.
 */
static cJSON* get_create_params(RPCJson_t* obj)
{
