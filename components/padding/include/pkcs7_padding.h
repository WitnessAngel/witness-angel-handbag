...
int pkcs7_padding_valid(uint8_t* buffer, size_t data_length, size_t buffer_size,
                        uint8_t modulus);

/* Given a block of pkcs7 padded data, return the actual data length in the
 * block based on the padding applied. buffer_size must be a multiple of modulus
